...
- `/sp/camera` (SP_3): カメラ/ファイル入力で撮影。撮影後に数量入力へ遷移。
- `/sp/count/:photoId` (SP_4): サムネ + 計算式 + 電卓型テンキー。確定/キャンセル。
- `/pc/assign` (PC_1): 未割り当て/割り当て済みタブ。写真カードグリッド、数量編集、商品割り当て/変更、削除。
- `/pc/assign/modal/:photoId` (PC_2): 商品割当モーダル。AI推薦(ダミー)、検索、簡易商品登録、選択確定。
- `/pc/assigned` (PC_3): 割当済み単票ビュー。
- `/pc/report` (PC_4): 棚卸表サマリー + テーブル + CSV出力ボタン。「棚卸完了する」はダミーアラート。
